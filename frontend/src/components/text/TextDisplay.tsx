...
const TextDisplay: React.FC<{ segmentId: string }> = ({ segmentId }) => {
    ...
    const expand = useCallback((direction: Direction) => {
        ...
        segmentsPromise.then(([response, _]) => {
            ...
            const ids = response.map(segment => ({ segmentId: segment.id, originalSegmentId: segment.original ? segment.original.id : undefined }));
    ...
    useLayoutEffect(() => {
        ...
        if (!once.current && offBoundsCount >= 2) {
            once.current = true;
            chosenRef.current.scrollIntoView({ behavior: "smooth" });
...

    {
        ...
        const handleUpObserver = useCallback((entries: IntersectionObserverEntry[]) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    rememberScroll.current = divRef.current.scrollHeight - divRef.current.scrollTop - divRef.current.clientHeight;
                    expand(Direction.UP);
                } else {
                    if (!once.current) {
        ...
        useLayoutEffect(() => {
            if (divRef.current.clientHeight < divRef.current.offsetHeight)
                return;
            divRef.current.scrollTop = divRef.current.scrollHeight - rememberScroll.current - divRef.current.clientHeight
        }, [segmentDisplayArray[0]])

        const handleDownObserver = useCallback((entries: IntersectionObserverEntry[]) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
    ...
    return <div ref={divRef} className={`text-display ${offBoundsCount >= 2 ? 'text-display__loaded' : ''}`}>
        {segmentDisplayArray.length > 0 &&
            <>
                <div ref={upperTriggerRef} id="upper-trigger" />
                {segmentDisplayArray.map(info => (
                    <span ref={segment.id === info.segmentId || segment.id === info.originalSegmentId ? chosenRef : null} key={`${info.segmentId}-${info.originalSegmentId}`}>
                        <TextSegmentDisplay
