...
const FilePeek: React.FC = () => {
    ...
    return <div className="file-peek">
        <div className="file-peek_process">
            <button onClick={() => api.splitFile(+fileId, regexpToSend).then()}>Обработать</button>
            <input disabled={regexpSelected != -2} onChange={handleChange} value={regexpString}></input>
            <select value={regexpSelected} onChange={handleSelect}>
                ...
                {regexpList.map((regexp, index) => (
                    <option value={index} key={index}>{regexp.name}</option>
                ))}
            </select>
        </div>
        <div className="file-peek_text">{markedText.map((e, index) =>
            <span key={index} className={`file-peek_text-segment ${e.marked ? `file-peek_marked_${index % 3}` : ''}`}>
                {e.text}
            </span>)}</div>
    </div>
}
